...
contract Domains is ERC721URIStorage{
    ...
    // helps us keep track of tokenIds
    ...
    // connects a string to an address
    mapping(string => address) public domains;

    //stores the user's records
    mapping(string => string) public records;

    constructor(string memory _tld) payable ERC721("Lincoln's Name Service", "ljtron"){
        tld = _tld;
    ...
    function price(string calldata name) public pure returns(uint){
        ...
        require(len > 0, "You must enter a name");
        if(len > 3){
            return 3 * (10**17);
        }
        else if(len == 4){
    ...
    function register(string calldata name) payable public{
        require(domains[name] == address(0));

        uint _price = price(name);
        require(msg.value >= _price, "Not enough Matic");

        // combines the name with the tld
        //abi.encodePacked allows you to combine strings because you can't concatatnate in solidity
        ...
        console.log("Registered %s. %s on the contract with tokenId %d", name, tld, newRecordId);
    ...
    function setRecord(string calldata name, string calldata _record) public{
